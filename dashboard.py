...
from external_api.reddit_api import reddit_api
...
def run_scheduler():
    ...
    # schedule.every().day.at("20:15").do(news.get_news_data)
    # schedule.every().tuesday.at("22:00").do(news.get_news_data)
    # schedule.every().tuesday.at("23:00").do(reddit_api.reddit_api)
    schedule.every().day.at("11:44").do(news.get_news_data)
    # schedule.every().day.at("11:16").do(reddit_api.reddit_api)
    
    # schedule.every(10).monday.at("02:00").do(notification.testin
    # g_function)
...
if __name__ == "__main__":
    ...
    #defining function of choosing either Gemini or Groq
    def choose_ai(prompt):
        try:
            # PRIMARY: Gemini
    ...
    with main_col:
        ...
        st.plotly_chart(fig_news, use_container_width=True)
            ...
            # home work
            # News Category Distribution (Exact Chart)

        st.subheader("News Coverage by Category")
        ...
        # Improve readability (important for many categories)
        fig_news_cat.update_layout(
            xaxis_tickangle=-45,
            xaxis_title="category",
            yaxis_title="article_count"
        )

        st.plotly_chart(fig_news_cat, use_container_width=True)
        ...
        st.dataframe(category_compare)
            ...
            # Dual Axis Bar Chart for Cross-Source Category Comparison
        import plotly.graph_objects as go

        fig = go.Figure()

        # Reviews (Primary Y-axis)
        fig.add_trace(
            go.Bar(
                x=category_compare["category"],
                y=category_compare["Review Mentions"],
                name="Review Mentions",
                yaxis="y1"
            )
        )

        # Reddit (Secondary Y-axis)
        fig.add_trace(
            go.Bar(
                x=category_compare["category"],
                y=category_compare["Reddit Mentions"],
                name="Reddit Mentions",
                yaxis="y2"
            )
        )

        # News (Secondary Y-axis)
        fig.add_trace(
            go.Bar(
                x=category_compare["category"],
                y=category_compare["News Mentions"],
                name="News Mentions",
                yaxis="y2"
            )
        )
        ...
        fig.update_layout(
            title="Cross-Source Category Comparison (Dual Axis)",
            xaxis=dict(title="Category", tickangle=-45),

            yaxis=dict(
                title="Review Mentions (High Volume)",
                showgrid=True
            ),

            yaxis2=dict(
                title="Reddit & News Mentions (Low Volume)",
                overlaying="y",
                side="right",
                showgrid=False
            ),
        ...
        st.plotly_chart(
            ...
            key="dual_axis_category_comparison"
        )
